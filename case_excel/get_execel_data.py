...
from excel_column import excel_column
...
class GetData:
    def __init__(self):
        ...
        self.table = self.excel.get_table(self.excel.workbook)
    ...
    def get_case_name(self,row):
        ...
        case_name = self.excel.get_cell_value(self.table, row, col)
        return case_name

    # 获取用例di
    def get_case_id(self,row):
        col = excel_column.getcolumn_case_id()
        case_id = self.excel.get_cell_value(self.table,row,col)
        return case_id

    #获取是否执行
    def get_isrun(self,row):
        col = excel_column.getcolumn_isrun()
        run_modul = self.excel.get_cell_value(self.table,row,col)
        flag = None
    ...
    def get_method(self,row):
        ...
        request_method = self.excel.get_cell_value(self.table,row,col)
        return request_method

    #获取接口地址
    def get_url(self,row):
        col = excel_column.getcolumn_URL()
        url = self.excel.get_cell_value(self.table,row,col)
        if 'http' not in url:
            url = GLOBAL_URL + url
        return url
    ...
    def get_depend_caseid(self,row):
        ...
        depend_caseid = self.excel.get_cell_value(self.table,row,col)
        return depend_caseid

    #获取header
    def get_header(self,row):
        col = excel_column.getcolumn_header()
        header = self.excel.get_cell_value(self.table,row,col)
        return header

    #获取data请求数据
    def get_data(self,row):
        col = excel_column.getcolumn_data()
        data = self.excel.get_cell_value(self.table,row,col)
        return data

    #获取json请求数据
    def get_json(self,row):
        col = excel_column.getcolumn_json()
        json = self.excel.get_cell_value(self.table,row,col)
        return json

    #获取逾期结果
    def get_expend_result(self,row):
        col = excel_column.getcolumn_expend_result()
        expend_result=self.excel.get_cell_value(self.table,row,col)
        return expend_result

    #获取数据处理方法
    def get_data_deal(self,row):
        col = excel_column.getcolumn_data_deal()
        deal_method = self.excel.get_cell_value(self.table,row,col)
        return deal_method

    #获取依赖数据所属字段
    def get_depend_key(self,row):
        col = excel_column.getcolumn_depend_key()
        depend_key = self.excel.get_cell_value(self.table,row,col)
        return depend_key

    #写入逾期结果
    def set_actual_result(self,row,value):
        col = excel_column.getcolumn_actual_result()
        workbook = xlrd.open_workbook('../case_excel/case.xls')
        self.excel.write_execel(workbook,0,row,col,value)
    ...
    def get_actual_result(self,row):
        col = excel_column.getcolumn_actual_result()
        actual_result = self.excel.get_cell_value(self.table, row, col)
