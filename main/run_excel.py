    ...
    def go_on_run(self):
        ...
        if case_list[0] == '':
            ...
            for i in range(3, lines):
                execute_case.execute_one_case(i, self.pass_list, self.fail_list, self.case_run_list, self.case_i_dict,
                                              self.has_run_list)
        else:
            # 执行指定用例
            for y in case_list:
                execute_case.execute_one_case(self.case_i_dict[y], self.pass_list, self.fail_list, self.case_run_list,
                                              self.case_i_dict, self.has_run_list, True)

        # 将测试结果发送报告
        opreaemail = OpreationEmail()
        receivers = ['guojx@citycloud.com.cn']
        subject = '自动化测试报告'
        content = '用例总数' + str(lines - 2) + '条,执行' + str(len(self.case_run_list)) + '条,通过' + str(len(self.pass_list)) + \
                  '条,失败' + str(len(self.fail_list)) + '条' + '\n' + '通过率为 ' + str(
            len(self.pass_list) * 100 / len(self.case_run_list)) + '%'
        opreaemail.SMTP_QQ_send_email(receivers, subject, content, 'plain', '../case_excel/case.xlsx')
